...
//Vs fitness function variance, K carring capacity,
    short y2, i, j, n, xx, yy, jj[4];
    double dist, dist2;
    short tot = 0;
    short xg = (short)ceil((double)(xp) / 200.);
    short yg = (short)ceil((double)(yp) / 200.);
    short ff = xg - 1;
    short r = xg + 1;
    short t = yg - 1;
    short b = yg + 1;
...
void Newball2008(short n, short male, std::list<Cball>* list1, double fr) {
    ...
    for (j = 11 + (nloci - nopoly) / 2; j <= 10 + (nloci - nopoly) / 2 + nopoly; j++) {
        GerateRandomperm(n, tur);
        for (i = 1; i <= aa; i++) {
            ge1[tur[i]] = 1;
            ge2[tur[i]] = 1;
...
void AssignBucket(std::list<Cball>* list1) {
    ...
    nogy = yrange / 200;//the max number of y dimention of the bucket girds y = 1000 y = 5
    for (xc = 0; xc <= nogx; xc++) {
        for (yc = 0; yc <= nogy; yc++) {
    ...
    for (std::list<Cball>::iterator it = list1->begin(); it != list1->end(); ++it) {
        xc = (short)ceil(it->xp / 200.);
        yc = (short)ceil(it->yp / 200.);
